...
class extractFreeSurferStats():
    ...
    def dumpWGStats(self, subject):
        stats_csv = ''
        stats_file = ''
        #setup the table info
        tbl = self.tblroot + '_wgpct_stats'
        try:
            self.cursor.execute("""create table %s (%s varchar(50))""" % (tbl, self.id_col))
        except:
            pass
        ...
        for hemi in hemis:
            stats_dir = os.path.join(self.subjects_dir, subject, 'stats')
            stats_file = open(os.path.join(stats_dir, hemi + '.w-g.pct.stats'), 'r')
            stats_csv = csv.reader(stats_file, delimiter=' ')
            for row in stats_csv:
                if row[0] != '#':
                    for i in range(0, len(row)):
                    ...
                    try:
                        ...
                        snr = -1
                    col_mean = hemi + '_' + roi + '_wgpct_avg' + self.varSfix
                    col_stdev = hemi + '_' + roi + '_wgpct_std' + self.varSfix
                    col_snr = hemi + '_' + roi + '_wgpct_snr' + self.varSfix
                    try:
                        self.cursor.execute("""alter table %s add column (%s float)""" % (tbl, col_mean))
                    except:
                        pass
                    try:
                        self.cursor.execute("""alter table %s add column (%s float)""" % (tbl, col_stdev))
                    except:
                        pass
                    try:
                        self.cursor.execute("""alter table %s add column (%s float)""" % (tbl, col_snr))
                    except:
                        pass
                    self.cursor.execute("""update %s set %s=%s where %s=\'%s\'""" % (tbl, col_mean, avg, self.id_col, subject))
                    self.cursor.execute("""update %s set %s=%s where %s=\'%s\'""" % (tbl, col_stdev, stdev, self.id_col, subject))
                    self.cursor.execute("""update %s set %s=%s where %s=\'%s\'""" % (tbl, col_snr, snr, self.id_col, subject))
            stats_file.close()
